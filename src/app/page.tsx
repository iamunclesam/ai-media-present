...
export default function Home() {
  ...
  // Categories
  const effectiveOrgId = orgId ?? current?.org?._id;
  const categories = useQuery(
    api.categories.listByOrg,
    effectiveOrgId ? { orgId: effectiveOrgId } : "skip"
  );
  ...
  useEffect(() => {
    ...
    if (current === null) {
      ...
      void ensureCurrent()
        ...
        .catch(() => {
          setAuthError(
            "Convex auth is not configured. Check CLERK_JWT_ISSUER_DOMAIN."
          );
        })
        .finally(() => {
  ...
  const handleSelectSlide = useCallback(
    async (songId: Id<"songs">, index: number, slideText: string) => {
  ...
  const handleAddToService = async (songId: Id<"songs">) => {
    ...
    try {
      ...
      setActionError(
        error instanceof Error ? error.message : "Failed to add to service."
      );
    }
  };
  ...
  const handleRename = async () => {
    ...
    try {
      if (renameTarget.type === "service") {
        await renameService({
          serviceId: renameTarget.id as Id<"services">,
          name: renameTarget.name,
        });
      } else if (renameTarget.type === "category") {
        await renameCategory({
          categoryId: renameTarget.id as Id<"categories">,
          name: renameTarget.name,
        });
      } else if (renameTarget.type === "song") {
        await updateSong({
          songId: renameTarget.id as Id<"songs">,
    ...
    try {
      if (deleteTarget.type === "service") {
        ...
        await removeCategory({
          categoryId: deleteTarget.id as Id<"categories">,
        });
                ...
                {serviceItems.length > 0 ? (
                  serviceItems.map((item) => (
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => handleSelectServiceItem(item.index)}
                      ...
                      <button
                        ...
                        onClick={() => handleRemoveFromService(item.index)}
          ...
          <div className="flex-1 overflow-auto p-4">
            {viewMode === "show" ? (
                  ...
                  {slidesForGrid.map(({ song, slide, index }) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleSelectSlide(song._id, index, slide.text)
                        }
                        className={cn(
                          "relative flex aspect-video flex-col overflow-hidden rounded-lg border transition",
                          isActive
                      ...
                      >
                        <div className="flex flex-1 items-center justify-center bg-card p-2">
                          <p className="line-clamp-4 whitespace-pre-line text-center text-[11px] leading-snug text-foreground">
                            {slide.text}
                          </p>
                        </div>
                        <div
                          className={cn(
                            "px-2 py-1 text-[9px] font-semibold text-white text-center",
              ...
              <div className="flex h-full flex-col gap-3">
                ...
                {actionMessage && (
                  <p className="text-xs text-green-400">{actionMessage}</p>
                )}
                <textarea
                  value={editLyrics}
              ...
              <div className="flex items-center gap-1 px-2">
                {(["shows", "media", "scripture"] as BottomTab[]).map((tab) => (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-2 px-4 py-2 text-xs font-medium capitalize transition",
                      bottomTab === tab
                  ...
                  {filteredSongs.map((song) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelected({ songId: song._id, index: 0 });
                          }}
                      ...
                      {isInsideService && selectedServiceId && (
                        <button
                          type="button"
                          onClick={() => handleAddToService(song._id)}
        ...
        <aside className="flex w-56 shrink-0 flex-col border-l border-border bg-card">
          <div className="border-b border-border p-3">
            ...
            <div className="aspect-video rounded-lg border border-primary/50 bg-black flex items-center justify-center p-2">
              {activeSlide?.slide ? (
                <p className="whitespace-pre-line text-center text-[10px] leading-snug text-white font-medium">
                  {activeSlide.slide.text}
                </p>
              ) : (
          ...
          <div className="flex-1 overflow-auto p-3">
            ...
            <div className="space-y-1">
              {slideGroups.map((group) => (
                <div
                  key={group.label}
                  className="flex items-center justify-between rounded-md px-2 py-1 text-xs text-foreground hover:bg-secondary"
                >
